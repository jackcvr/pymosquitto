...
LIBMOSQ_VERSION = (C.c_int(), C.c_int(), C.c_int())
libmosq.mosquitto_lib_version(
    C.byref(LIBMOSQ_VERSION[0]),
    C.byref(LIBMOSQ_VERSION[1]),
    C.byref(LIBMOSQ_VERSION[2]),
)

LIBMOSQ_MIN_MAJOR_VERSION = 2
if LIBMOSQ_VERSION[0].value < LIBMOSQ_MIN_MAJOR_VERSION:
    raise Exception(f"libmosquitto version {LIBMOSQ_MIN_MAJOR_VERSION}+ is required")
